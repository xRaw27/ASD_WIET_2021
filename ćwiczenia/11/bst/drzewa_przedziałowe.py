...
def print_BST(root, l, r):
    if root is not None:
        ...
        print("key:", str(root.key).ljust(6, " "), " left:", str(left).ljust(6, " "), " right:", str(right).ljust(6, " "), " parent:", str(parent).ljust(6, " "), " interval: ", (str(l) + " " + str(r)).ljust(12, " "), "intervals: ", root.intervals)
...
def insert_interval(index, interval, root, l, r):
    print(interval, root.key, l, r)
    if interval[0] == l and interval[1] == r:
        root.intervals.append(index)
        return

    if interval[0] < root.key:
        insert_interval(index, (interval[0], min(root.key, interval[1])), root.left, l, root.key)

    if interval[1] > root.key:
        insert_interval(index, (max(root.key, interval[0]), interval[1]), root.right, root.key, r)
